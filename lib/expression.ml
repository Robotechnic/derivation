...
(** Display parenthesis if the child has a lower priority than the parent e.g. 2
    * (3 + 4) instead of 2 * 3 + 4

    @param [1] parent expression
    @param [2] child expression
    @param [3] string representation of the child expression
    @return
      string representation of the child expression with parenthesis if needed *)
let parentheses parent child str =
  let childPriority = priority child in
  if childPriority <> 0 && priority parent > childPriority
...
let add e1 e2 =
  ...
  | Var (coeff1, s1, pow1), Var (coeff2, s2, pow2) when s1 = s2 && pow1 = pow2
    -> cpvar (coeff1 + coeff2) s1 pow1
...
let rec div e1 e2 =
  ...
  | Var (coeff1, s1, pow1), Var (coeff2, s2, pow2) when s1 = s2 ->
      if coeff1 mod coeff2 = 0
      then cpvar (coeff1 / coeff2) s1 (pow1 - pow2)
      else mul (Div (Number coeff1, Number coeff2)) (cpvar 1 s1 (pow1 - pow2))
  | Var (coeff1, s1, pow1), Number n2 when coeff1 mod n2 = 0 ->
      cpvar (coeff1 / n2) s1 pow1
  | Number n1, Number n2 when n1 mod n2 = 0 -> Number (n1 / n2)
...
and mul e1 e2 =
  ...
  | _, Number 1 -> e1
  | Number n1, Number n2 -> Number (n1 * n2)
  | Number n1, Var (coeff2, s2, pow2) | Var (coeff2, s2, pow2), Number n1 ->
      cpvar (coeff2 * n1) s2 pow2
  | Number n1, Div (Number n2, e2) | Div (Number n2, e2), Number n1 ->
      div (Number (n1 * n2)) e2
...
let pow e1 e2 =
  ...
  | _, Number 1 -> e1
  | Number n1, Number n2 ->
      Number (int_of_float (float_of_int n1 ** float_of_int n2))
  | _, _ -> Pow (e1, e2)
...
let derivateFunc name expr =
  ...
  | "asin" ->
      div (Number 1) (Func ("sqrt", Sub (Number 1, Pow (var "x", Number 2))))
  | "acos" ->
      neg
        (div (Number 1)
           (Func ("sqrt", Sub (Number 1, Pow (var "x", Number 2)))))
...
let rec derivate expression variable =
  ...
  | Var (coeff, name, power) ->
      if name = variable
      then cpvar (coeff * power) name (power - 1)
      else Number 0
  | Number _ -> Number 0
  | Add (e1, e2) -> add (derivate e1 variable) (derivate e2 variable)
  | Sub (e1, e2) -> sub (derivate e1 variable) (derivate e2 variable)
  | Mul (e1, e2) ->
      add (mul (derivate e1 variable) e2) (mul e1 (derivate e2 variable))
  | Div (e1, e2) ->
      div
        (sub (mul (derivate e1 variable) e2) (mul e1 (derivate e2 variable)))
        (pow e2 (Number 2))
  | Pow (e1, Number n) ->
      mul (mul (Number n) (derivate e1 variable)) (pow e1 (Number (n - 1)))

...
| MINUS (p)::t -> (
  try 
    let (exp, rest) = factor t in (neg exp, rest)
  with SyntaxError (ep, error) -> 
    if ep = -1 then raise (SyntaxError (p, "Expected an expression after '-'"))
    else raise (SyntaxError (ep, error))
)
| PLUS (p)::t -> (
  try
    let (exp, rest) = factor t in (exp, rest)
  with SyntaxError (ep, error) ->
    if ep = -1 then raise (SyntaxError (p, "Expected an expression after '+'"))
    else raise (SyntaxError (ep, error))
)
